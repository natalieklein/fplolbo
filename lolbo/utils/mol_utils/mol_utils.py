...
def smile_to_tdc_docking_score(smiles_str, tdc_oracle, max_smile_len=600, timeout=600):
    ...
    print(
        f"getting docking score: {ret_value} from protein took {time.time()-start} seconds"
    )
    return ret_value
...
def get_fps_to_list_of_targets(fp1, sz1, target_fps, target_szs):
    return np.array(
        [
            get_fps_efficient(fp1, sz1, target_fps[ix], target_szs[ix])
            for ix in range(len(target_fps))
        ]
    )
    # TEST SCRIPT:
