...
class AAETrainer(MosesTrainer):
    ...
    def _pretrain_epoch(self, model, tqdm_data, criterion, optimizer=None):
        ...
        for i, (encoder_inputs, decoder_inputs, decoder_targets) in enumerate(
            tqdm_data
        ):
        ...
        postfix["mode"] = (
            "Pretraining:Eval" if optimizer is None else "Pretraining:Train"
        )
    ...
    def _train(self, model, train_loader, val_loader=None, logger=None):
        ...
        schedulers = {
            k: torch.optim.lr_scheduler.StepLR(
                v, self.config.step_size, self.config.gamma
            )
