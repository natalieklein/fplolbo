...
class ORGANTrainer(MosesTrainer):
    ...
    def _pretrain_generator(self, model, train_loader, val_loader=None, logger=None):
        ...
        for epoch in range(self.config.generator_pretrain_epochs):
            tqdm_data = tqdm(
                train_loader, desc="Generator training (epoch #{})".format(epoch)
            )
            ...
            if val_loader is not None:
                tqdm_data = tqdm(
                    val_loader, desc="Generator validation (epoch #{})".format(epoch)
                )
    ...
    def _pretrain_discriminator_epoch(
        self, model, tqdm_data, criterion, optimizer=None
    ):
        model.generator.eval()
        if optimizer is None:
            model.discriminator.eval()
        ...
        for i, inputs_from_data in enumerate(tqdm_data):
            ...
            inputs_from_model, _ = model.sample_tensor(
                self.config.n_batch, self.config.max_length
            )
    ...
    def _pretrain_discriminator(
        self, model, train_loader, val_loader=None, logger=None
    ):
        device = model.device
        ...
        for epoch in range(self.config.discriminator_pretrain_epochs):
            tqdm_data = tqdm(
                train_loader, desc="Discriminator training (epoch #{})".format(epoch)
            )
    ...
    def _policy_gradient_iter(self, model, train_loader, criterion, optimizer, iter_):
        ...
        for _ in gen_tqdm:
            ...
            mean_episode_reward = (
                torch.cat([t[:l] for t, l in zip(rewards, lengths)]).mean().item()
            )
    ...
    def _train_policy_gradient(self, model, train_loader, logger=None):
        ...
        optimizer = {
            "generator": torch.optim.Adam(
                model.generator.parameters(), lr=self.config.lr
            ),
        ...
        for iter_ in range(self.config.pg_iters):
            postfix = self._policy_gradient_iter(
                model, train_loader, criterion, optimizer, iter_
            )
            if logger is not None:
                logger.append(postfix)
                logger.save(self.config.log_file)
    ...
    def fit(self, model, train_data, val_data=None):
        ...
        gen_train_loader = self.get_dataloader(
            model, train_data, gen_collate_fn, shuffle=True
        )
        gen_val_loader = (
            None
            if val_data is None
        ...
        dsc_train_loader = self.get_dataloader(
            model, train_data, dsc_collate_fn, shuffle=True
        )
        dsc_val_loader = (
            None
            if val_data is None
        ...
        if model.metrics_reward is not None:
            (self.ref_smiles, self.ref_mols) = model.metrics_reward.get_reference_data(
                train_data
            )
