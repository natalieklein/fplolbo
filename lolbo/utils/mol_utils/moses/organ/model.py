...
class Discriminator(nn.Module):
    def __init__(self, embedding_layer, convs, dropout=0):
        ...
        self.conv_layers = nn.ModuleList(
            [
                nn.Conv2d(1, f, kernel_size=(n, self.embedding_layer.embedding_dim))
                for f, n in convs
            ]
        )
...
class ORGAN(nn.Module):
    ...
    def rollout(self, n_samples, n_rollouts, ref_smiles, ref_mols, max_len=100):
        with torch.no_grad():
            ...
            prevs = torch.empty(
                n_samples, 1, dtype=torch.long, device=self.device
            ...
            for current_len in range(max_len):
                ...
                rollout_sequences = torch.cat(
                    [s[~is_end, :].repeat(n_rollouts, 1) for s in sequences]
                    + [rollout_sequences],
                    dim=-1,
                )
                ...
                current_rewards[~is_end] = rollout_rewards.view(n_rollouts, -1).mean(
                    dim=0
                )
    ...
    def sample_tensor(self, n, max_len=100):
        prevs = torch.empty(n, 1, dtype=torch.long, device=self.device).fill_(
            self.vocabulary.bos
        )
