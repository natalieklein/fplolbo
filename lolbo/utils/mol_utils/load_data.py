...
def load_molecule_train_data(
    ...
    df = pd.read_csv(
        "../lolbo/utils/mol_utils/guacamol_data/guacamol_train_data_first_20k.csv"
    )
...
def load_train_z(
    ...
    path_to_init_train_zs = path_to_vae_statedict.replace(
...
def compute_train_zs(mol_objective, init_train_x, bsz=64):
    ...
    state_dict_file_type = mol_objective.path_to_vae_statedict.split(".")[
        -1
    ]  # usually .pt or .ckpt
    path_to_init_train_zs = mol_objective.path_to_vae_statedict.replace(
        f".{state_dict_file_type}", "-train-zs.csv"
    )
